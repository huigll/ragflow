...
RUN --mount=type=cache,id=ragflow_apt,target=/var/cache/apt,sharing=locked \
    apt update && \
    apt install -y build-essential clang libpython3-dev libicu-dev libgbm-dev && \
    rm -rf /var/lib/apt/lists/*
...
RUN --mount=type=cache,id=ragflow_uv,target=/root/.cache/uv,sharing=locked \
    ...
    # Python 3.13 resolves NumPy 1.26.4 from source because several locked
    # dependencies require NumPy <2. GCC 13 can ICE while compiling it.
    CC=clang CXX=clang++ uv sync --python 3.13 --frozen --refresh-package litellm && \
